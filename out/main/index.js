...
async function initDatabase() {
  ...
  ipcMain.handle("db:saveProducto", (_, p) => {
    ...
    if (p.id) {
      db.run(
        `UPDATE productos SET codigo=?, nombre=?, precio=?, unidad=?, updated_at=? WHERE id=?`,
        [p.codigo, p.nombre, p.precio, p.unidad, now, p.id]
      );
    } else {
      db.run(
        `INSERT INTO productos (codigo, nombre, precio, unidad, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`,
  ...
  ipcMain.handle("db:saveHistorial", (_, data) => {
    const now = getLocalDateTime();
    db.run(
      `INSERT INTO historial_impresiones (producto_id, plantilla_id, cantidad, estado, fecha) VALUES (?, ?, ?, ?, ?)`,
  ...
  ipcMain.handle("excel:import", async (_, filePath, sheetIndex) => {
    ...
    for (let i = 1; i < allRows.length; i++) {
      ...
      let precio = 0;
      if (values[colMap.precio] !== void 0 && values[colMap.precio] !== null) {
      ...
      if (existing.length === 0 || existing[0].values.length === 0) {
        ...
        const existingObj = {};
        existing[0].columns.forEach((c, idx) => {
          existingObj[c] = existing[0].values[0][idx];
        });
        const changed = existingObj.nombre !== nombre || existingObj.precio !== precio || existingObj.unidad !== unidad;
        if (changed) {
          db.run(
            `UPDATE productos SET nombre=?, precio=?, unidad=?, updated_at=? WHERE codigo=?`,
